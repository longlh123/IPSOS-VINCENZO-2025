...
class NPSBarChartWidget(QWidget):
    ...
    def plot_nps_bar_chart(self):
        ...
        bar_width = 0.3
        bar_gap = 0.1  # thêm khoảng cách giữa các wave
        group_width = (bar_width + bar_gap) * wave_count + 0.1
        ...
        for i, wave in enumerate(waves):
            ...
            ax.bar(x_pos, promoter, bar_width, color=colors[0], label=f"{wave} - Promoter" if i == 0 else None)
            ax.bar(x_pos, passive, bar_width, bottom=promoter, color=colors[1], label=f"{wave} - Passive" if i == 0 else None)
            ax.bar(x_pos, detractor, bar_width, bottom=promoter + passive, color=colors[2], label=f"{wave} - Detractor" if i == 0 else None)

            for xi, yi in zip(x_pos, nps):
                ax.text(xi, 102, f"{yi}", ha="center", va="center", fontsize=9, color="white",
